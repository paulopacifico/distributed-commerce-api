...
class PaymentSagaEventPublisher(
    ...
    fun publishSucceeded(event: PaymentSucceededEvent) {
        publish(topic = topics.paymentSucceeded, key = event.orderNumber, event = event, eventName = "PaymentSucceededEvent")
    }

    fun publishFailed(event: PaymentFailedEvent) {
        publish(topic = topics.paymentFailed, key = event.orderNumber, event = event, eventName = "PaymentFailedEvent")
    }
